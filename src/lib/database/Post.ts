...
export async function getTrendPostList(
  ...
  try {
    ...
    // クエリパラメータを用意
    const params: { [key: string]: unknown } = { count };
    ...
    return response.data as ItemInfo[];
  } catch (error) {
...
//   try {
//     // クエリパラメータを用意
//     const params: { [key: string]: unknown } = { postId, userId, like };
...
export async function getLikeListByUserId(
  ...
  try {
    ...
    const likePosts = response.data.map((res: { post: ItemInfo }) => {
      return res.post;
    });
    return likePosts as ItemInfo[];
  } catch (error) {
    console.error("Error fetching data:", error);
